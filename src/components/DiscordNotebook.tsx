...
export function DiscordNotebook({ isGM, currentUserProfile, characters, onAddLog }: DiscordNotebookProps) {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (discordTargetId) {
        fetch(`${import.meta.env.VITE_API_URL || ''}/api/discord/notebook/send`, {
          method: 'POST',
  ...
  const handleDetectDiscordChannel = async (idToDetect?: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/discord/channel-info?channelId=${encodeURIComponent(targetId)}`);
